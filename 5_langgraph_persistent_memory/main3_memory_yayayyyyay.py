...
rag_prompt = ChatPromptTemplate.from_messages([
    ("system", "Use the following context to answer the user's question. "
                ...
                "Do not make up facts. Context: {context}"),
    MessagesPlaceholder(variable_name="messages")
])
...
graph.add_edge("web_search", END)  # chain to rag
...
def run_chat():
    ...
    while True:
        ...
        final_state = None
        try:
            for step in app.stream(state, config={"configurable": {"thread_id": "user-1"}}):
                final_state = step
        ...
        if branch in {"llm", "rag"}:
            ...
            state["messages"].append(AIMessage(content=f"{label} {response}".strip()))
